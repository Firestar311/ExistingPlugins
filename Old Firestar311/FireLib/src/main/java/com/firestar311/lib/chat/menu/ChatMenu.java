package com.firestar311.lib.chat.menu;

import com.firestar311.lib.chat.menu.element.ButtonElement;
import com.firestar311.lib.chat.menu.element.Element;
import com.firestar311.lib.chat.protocol.PlayerChatIntercept;
...
public class ChatMenu implements IElementContainer {
    ...
    public void unregister() {
        if (!registered) { throw new IllegalStateException("Menu not registered"); }
    ...
    public void edit(Player player, int elementIndex, String[] args) {
        ...
        if (element.onClick(this, player)) { refresh(); }
    }
...
//			System.err.println("WARNING! Potential overlap detected.");
        ...
        for (int i = 0; i < 20; i++) { lines.add(new Text()); }
    ...
    public void close(Player player) {
        ...
        if (viewers.size() == 0 && autoUnregister) { unregister(); }
    }
    
    void onClosed(Player player) {
        if (viewers.remove(player)) { ChatMenuAPI.getChatIntercept(player).resume(); }
    }
    ...
    public void openFor(Player player) {
        ...
        if (viewers.add(player) && pauseChat) { chat.pause(); }
        for (BaseComponent[] line : build()) { chat.sendMessage(line); }
    ...
    public void refresh() {
        ...
        for (Player viewer : viewers) { openFor(viewer); }
    }
    ...
    public String getCommand() {
        if (!isRegistered()) { throw new IllegalStateException("Unregistered menus can't be interacted with."); }
        return "/cmapi " + id + " ";
    }
