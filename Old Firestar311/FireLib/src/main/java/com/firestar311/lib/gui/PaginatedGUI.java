...
public class PaginatedGUI implements InventoryHolder {
    ...
    public PaginatedGUI(Plugin plugin, String name) {
        this(plugin, name, true);
    }
    
    public PaginatedGUI(Plugin plugin, String name, boolean paginated) {
        this(plugin, name, paginated, 54);
    }
    
    public PaginatedGUI(Plugin plugin, String name, boolean paginated, int maxSlots) {
        this(plugin, name, paginated, maxSlots, false);
    }
    
    public PaginatedGUI(Plugin plugin, String name, boolean paginated, int maxSlots, boolean allowInsert) {
        items = new HashMap<>();
    ...
    public GUIButton getButton(int slot) {
        if (slot < 45) {
            ...
            int index = slot;
            if (paginated) {
                if (page > 0) index = (page * 44) + slot + 1;
            }
    ...
    public Inventory getInventory() {
        ...
        if (paginated) {
            ...
            /* BEGIN PAGINATION */
            GUIButton backButton = new GUIButton(new ItemBuilder(Material.ARROW).withName("&aPrevious Page").buildItem());
            GUIButton pageIndicator = new GUIButton(new ItemBuilder(Material.NAME_TAG).withName("&aPage " + (currentPage + 1) + " of " + (this.getFinalPage() + 1)).buildItem());
            GUIButton nextButton = new GUIButton(new ItemBuilder(Material.ARROW).withName("&aNext Page").buildItem());
