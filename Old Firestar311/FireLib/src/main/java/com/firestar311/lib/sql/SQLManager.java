...
public class SQLManager {
    ...
    public SQLManager(JavaPlugin plugin, DBType type, ConfigurationSection dbSection) {
        ...
        if (!logFile.exists()) {
            try {
                ...
                plugin.getLogger().severe("Could not create the queries.txt file.");
            }
        }
    ...
    public boolean establishConnection() {
        ...
        try {
            switch (type) {
                case MYSQL: connectionURL = type.getUrl().replace("{hostname}", this.hostname).replace("{port}", this.port + "")
                        .replace("{database}", this.database);
                    break;
                case SQLITE: connectionURL = type.getUrl().replace("{file}", this.sqliteFile + "");
                    break;
            }
    ...
    public void pushQueueChanges() {
        for (IRecord record : queue) {
            ...
            for (int i = 0; i < declaredFields.length; i++) {
                ...
                try {
                    valueBuilder.append("'").append(field.get(record)).append("'");
                    if (i < (declaredFields.length - 1)) {
                        colBuilder.append(",");
                        valueBuilder.append(",");
                    }
            ...
            String sql = Statements.INSERT.replace("{name}", table).replace("{columns}", colBuilder.toString()).replace("{values}", valueBuilder.toString());
    ...
    public Set<IRecord> getRecords(String tableName, ResultSet resultSet) throws Exception {
        ...
        while (true) {
            ...
            IRecord recordObject = (IRecord) recordClass.getDeclaredConstructor().newInstance();
