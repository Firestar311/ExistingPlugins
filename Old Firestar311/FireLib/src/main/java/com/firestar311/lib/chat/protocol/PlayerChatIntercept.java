package com.firestar311.lib.chat.protocol;
...
public class PlayerChatIntercept extends ChannelDuplexHandler {
    ...
    PlayerChatIntercept(ChatPacketInterceptor interceptor, Player player) {
        ...
        while (messageQueue.size() < 20) messageQueue.add(new BaseComponent[0]);
    }
    ...
    public void write(ChannelHandlerContext context, Object packet, ChannelPromise promise) throws Exception {
        ...
        if (components != null) {
            ...
            if (!paused || !allowed) {
                while (messageQueue.size() > 20) messageQueue.remove();
