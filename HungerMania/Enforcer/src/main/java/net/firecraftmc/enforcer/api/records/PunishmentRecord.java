package net.firecraftmc.enforcer.api.records;
...
public class PunishmentRecord implements IRecord<Punishment> {
    ...
    public PunishmentRecord(Row row) {
        this.object = Punishment.builder().id(row.getInt("id")).actor(row.getString("actor")).target(row.getString("target")).reason(row.getString("reason"))
                .pardonReason(row.getString("pardonReason")).server(row.getString("server")).type(Punishment.Type.valueOf(row.getString("type")))
                .date(row.getLong("date")).pardonDate(row.getLong("pardonDate")).length(row.getLong("length")).visibility(Punishment.Visibility.valueOf(row.getString("visibility")))
                .pardonVisibility(Punishment.Visibility.valueOf(row.getString("pardonVisibility"))).active(row.getBoolean("active")).acknowledged(row.getBoolean("acknowledged")).build();
    }
