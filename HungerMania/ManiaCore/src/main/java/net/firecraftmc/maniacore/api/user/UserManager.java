...
public abstract class UserManager {
    public User getUser(String name) {
        ...
        if (user == null) {
            ...
            List<IRecord> records = CenturionsCore.getInstance().getDatabase().getRecords(UserRecord.class, "name", name);
            ...
            if (!records.isEmpty()) {
                ...
                user = constructUser(CenturionsUtils.getUUIDFromName(name), name);
                System.out.println("User is " + user);
                CenturionsCore.getInstance().getDatabase().pushRecord(new UserRecord(user));
    ...
    public void loadIgnoredPlayers(User user) {
        List<IRecord> records = CenturionsCore.getInstance().getDatabase().getRecords(IgnoreInfoRecord.class, "player", user.getUniqueId().toString());
    ...
    public User getUser(int userId) {
        ...
        if (user == null) {
            List<IRecord> records = CenturionsCore.getInstance().getDatabase().getRecords(UserRecord.class, "id", userId);
            if (!records.isEmpty()) {
                user = constructUser(((UserRecord) records.get(0)).toObject());
                Redis.pushUser(user);
            }
    ...
    public User getUser(UUID uuid) {
        ...
        if (!redisData.isEmpty()) {
            ...
            List<IRecord> userRecords = CenturionsCore.getInstance().getDatabase().getRecords(UserRecord.class, "uniqueId", uuid.toString());
            if (!userRecords.isEmpty()) {
                ...
                user = constructUser(uuid, CenturionsUtils.getNameFromUUID(uuid));
            ...
            List<IRecord> statsRecords = CenturionsCore.getInstance().getDatabase().getRecords(StatRecord.class, "uuid", uuid.toString());
            ...
            List<IRecord> togglesRecords = CenturionsCore.getInstance().getDatabase().getRecords(ToggleRecord.class, "uuid", uuid.toString());
        ...
        for (Stat value : Stat.REGISTRY.values()) {
            if (!stats.containsKey(value.getName())) {
                ...
                CenturionsCore.getInstance().getDatabase().pushRecord(new StatRecord(statistic));
        ...
        if (user.getName() == null || user.getName().equals("") || user.getName().equals("null")) {
            ...
            user.setName(CenturionsUtils.getNameFromUUID(uuid));
            CenturionsCore.getInstance().getDatabase().pushRecord(new UserRecord(user));
        }
