...
public class SettingsManager {
    ...
    public void loadData() {
        List<IRecord> records = CenturionsCore.getInstance().getDatabase().getRecords(GameSettingRecord.class, null, null);
        ...
        if (records.isEmpty()) {
            for (Setting value : Setting.values()) {
                ...
                CenturionsCore.getInstance().getDatabase().addRecordToQueue(new GameSettingRecord(setting));
                settings.add("default", setting);
            }
            CenturionsCore.getInstance().getDatabase().pushQueue();
        } else {
        ...
        for (Setting setting : Setting.values()) {
            for (String key : settings.keySet()) {
                ...
                CenturionsCore.getInstance().getDatabase().addRecordToQueue(new GameSettingRecord(s));
                settings.add(key, s);
            }
        }
