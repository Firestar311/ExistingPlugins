...
import com.stardevmc.enforcer.objects.reports.Report.Outcome;
import com.stardevmc.enforcer.objects.reports.Report.Status;
...
public class PunishmentManager extends Manager {
    ...
    private Map<String, String> ackCodes = new TreeMap<>();
    private Map<String, Punishment> punishments = new TreeMap<>();
    ...
    public void saveData() {
        ...
        for (Entry<String, Punishment> entry : this.punishments.entrySet()) {
    ...
    public void addPunishment(Punishment punishment) {
        if (StringUtils.isEmpty(punishment.getId()) || this.punishments.containsKey(punishment.getId())) {
            String id;
            do {
                id = Code.generateNewCode(8);
            } while (this.punishments.containsKey(id));
            punishment.setId(id);
        }
        
        if (plugin.getTrainingModule().getManager().isTrainingMode(punishment.getActor())) {
        ...
        plugin.getReportModule().getManager().getReports().values().stream().filter(report -> report.getTarget().equals(punishment.getTarget())).filter(report -> report.getReason().equalsIgnoreCase(punishment.getReason())).forEach(report -> {
            ...
            report.setOutcome(Outcome.ACCEPTED);
    ...
    private boolean checkActive(Punishment punishment) {
        ...
        if (punishment.isActive()) {
            if (punishment.getActor() instanceof PlayerActor) {
                if (punishment.isTrainingPunishment()) {
                    return plugin.getTrainingModule().getManager().isTrainingMode(punishment.getActor());
                }
            }
            return true;
        }
    ...
    public void addJailPunishment(JailPunishment punishment) {
        ...
        if (punishment.isActive()) {
            try {
                ...
                System.out.println("Could not find a prison for an active jail punishment with id " + punishment.getId());
            }
        }
    }
    ...
    public void addAckCode(String id, String code) {
        this.ackCodes.put(id, code);
    }
    
    public String generateAckCode(String id) {
    ...
    public String getAckCode(UUID uuid) {
        for (String id : this.ackCodes.keySet()) {
                ...
                if (playerTarget.getUniqueId().equals(target)) {
                    if (punishment.getRuleId() == rule.getId()) {
                        ...
                        for (Punishment p : punishments) {
                            if (p.getViolationNumber() == punishment.getViolationNumber()) {
                                toAdd = false;
                            }
