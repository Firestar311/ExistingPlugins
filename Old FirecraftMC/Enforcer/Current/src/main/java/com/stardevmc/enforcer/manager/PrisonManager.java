...
public class PrisonManager extends Manager {
        ...
        if (allPrisonsFull) {
            for (Prison j : this.prisons.values()) {
                if (prison == null) {
                    ...
                    int amountOver1 = prison.getInmates().size() - prison.getMaxPlayers();
                    int amountOver2 = j.getInmates().size() - j.getMaxPlayers();
    ...
    public Set<Prison> getPrisonsWithOverflow() {
        ...
        for (Prison prison : getPrisons()) {
            if (prison.getInmates().size() > prison.getMaxPlayers()) {
                prisons.add(prison);
            }
        }
