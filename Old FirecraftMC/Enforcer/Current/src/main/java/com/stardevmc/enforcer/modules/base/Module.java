...
public abstract class Module<M extends Manager> {
    ...
    public Module(Enforcer plugin, String name, M manager, String... commands) {
        ...
        if (!plugin.getConfig().contains("module." + name.toLowerCase())) {
            plugin.getConfig().set("module." + this.name + ".enabled", true);
        }
        
        this.enabled = plugin.getConfig().getBoolean("module." + name.toLowerCase() + ".enabled");
    }
    ...
    public void enable() {
        setEnabled(true);
        setup();
        plugin.getConfig().set("module." + this.name + ".enabled", enabled);
    }
    ...
    public ConfigurationSection getSection() {
        if (!this.plugin.getConfig().contains("module." + this.name.toLowerCase().replace(" ", "_"))) {
            this.plugin.getConfig().createSection("module." + this.name.toLowerCase().replace(" ", "_"));
        }
        return this.plugin.getConfig().getConfigurationSection("module." + this.name.toLowerCase().replace(" ", "_"));
    }
