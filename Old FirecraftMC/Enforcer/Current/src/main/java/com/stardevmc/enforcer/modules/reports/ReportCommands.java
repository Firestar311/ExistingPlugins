    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (cmd.getName().equalsIgnoreCase("report")) {
            if (Utils.isInt(args[0])) {
                ...
                if (args.length == 1) {
                    ...
                    if (Utils.checkCmdAliases(args, 1, "setevidence", "se")) {
                        ...
                        if (report.getStatus().equals(Status.CANCELLED)) {
                            player.sendMessage(Utils.color("&cThat report is already cancelled."));
                            return true;
                        }
                        
                        if (report.getOutcome().equals(Outcome.ACCEPTED) || report.getOutcome().equals(Outcome.DENIED)) {
                            player.sendMessage(Utils.color("&cThat report has already been decided on, you cannot cancel it now."));
                            return true;
                        }
                        
                        report.setOutcome(Outcome.CANCELLED);
            ...
            if (Utils.checkCmdAliases(args, 1, "assign", "a")) {
                ...
                Status status;
                try {
                    status = Status.valueOf(args[2].toUpperCase());
                ...
                Outcome outcome;
                try {
                    outcome = Outcome.valueOf(args[2].toUpperCase());
                } catch (IllegalArgumentException e) {
                ...
                Outcome oldOutcome = report.getOutcome();
                ...
                Punishment IPunishment = plugin.getPunishmentModule().getManager().getPunishment(args[2]);
                if (IPunishment == null) {
                    player.sendMessage(Utils.color("&cThe id you provided does not match a valid punishment"));
                    return true;
                }
                
                report.addPunishment(IPunishment);
                player.sendMessage(Utils.color(String.format("&aAdded the punishment &b%s &ato the report &b%s", IPunishment.getId(), report.getId())));
            }
        }
